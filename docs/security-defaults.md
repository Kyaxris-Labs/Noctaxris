...
- Compose publishes `127.0.0.1:4566` (HTTP control plane and health) and `127.0.0.1:8443` (IoT device TLS). That is not `0.0.0.0` on the host.
- Inside the container the process listens on `0.0.0.0:4566` so the published mapping works, and on `0.0.0.0:8443` for `NOCTAXRIS_IOT_TLS_LISTEN`. Compose sets `NOCTAXRIS_ALLOW_NONLOOPBACK_LISTEN=1` for those binds. Do not remap host ports to `0.0.0.0` without TLS.
- Compose does **not** set `NOCTAXRIS_ALLOW_OPEN_DATA_PLANE`. Function URL / HTTP API `NONE` stay off on the container bind unless you opt in with `compose.lab-open.yaml`: `docker compose -f compose.yaml -f compose.lab-open.yaml up` from `docker/`. Loopback process listen still allows `NONE` without that env.
...
- Image pulls through DinD are allowlisted (lab registry `127.0.0.1:4566/...`, rewritten `host.docker.internal`, pinned public Lambda bases, and documented lab images such as `alpine:3.23`). Other registries fail closed unless listed in `NOCTAXRIS_IMAGE_PULL_ALLOWLIST` (digest pins required for registry hosts).
...
- Shared Mosquitto (when `NOCTAXRIS_SHARED_MQTT=1`) requires lab IoT CA client certificates (`require_certificate`, `allow_anonymous false`); no default MQTT password path. Device CONNECT ClientId must equal the attached thing name (dynsec `clientid` plus `AllowMQTTConnect`). Device ACL is thing-scoped; there is no `pattern readwrite #` for devices. IoT device policies are evaluated fail-closed on Connect/Publish/Subscribe/Receive. Shared Kafka is not multi-tenant across accounts (one MSK metadata row per process when shared mode is on).
...
- JSON 1.1 `X-Amz-Target` must match SigV4 credential-scope service (with signing-name aliases). Empty-action path-style S3 requires scope `s3`. IoT data-plane REST requires an IoT signing name (`iot` / `iotdata` / `iotdevicegateway` / `iot-jobs-data`). Query signed as `iam` or `sts` must match those APIs. Other Query `Action=` routes still use `verified.Service` or action prefix.
...
- Nested compute uses `github.com/moby/moby/client` + `github.com/moby/moby/api` (not the legacy `github.com/docker/docker` module). CI `govulncheck` has an empty Fixed-N/A allowlist by default (`scripts/govulncheck-allowlist.txt`); new app vulns still fail the job. Docker Scout / Hub Security Insights Highs that still name Engine CVEs against an old client path are stale attributions or nested `docker:*-dind` surfaces, not distroless OS package CVEs (`gcr.io/distroless/static-debian12:nonroot` scans clean). AuthZ-plugin bypass findings do not apply to the packaged nested engine path (no AuthZ plugins). Changing the API image OS / base does not clear Scout Highs that target the nested engine binary. `CopyToContainer` / `CopyFromContainer` are limited to CodeBuild workspace paths under `/codebuild/src` via `internal/compute` helpers; Lambda and related code still reach nested containers via read-only bind mounts. Residual is still the nested engine binary version and who can talk to it over TLS on the Compose network.
...
Set both `NOCTAXRIS_TLS_CERT` and `NOCTAXRIS_TLS_KEY` to serve TLS on the main listener. If either is empty, that listener uses plain HTTP. Compose health (`/_noctaxris/health` / `/_noctaxris/ready`) stays HTTP `:4566`. Enabling main-listener TLS without updating probe URLs or client endpoint URLs is a common footgun (clients keep calling `http://127.0.0.1:4566` while the listener expects HTTPS). The IoT device listener (`NOCTAXRIS_IOT_TLS_LISTEN`, Compose `:8443`) is a separate TLS server with `VerifyClientCertIfGiven` and does not change those HTTP probes. Non-loopback listen without TLS requires `NOCTAXRIS_ALLOW_NONLOOPBACK_LISTEN=1`. Example keys under `docker/` or docs are for local labs only; do not reuse them outside the lab.
