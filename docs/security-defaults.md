...
- Nested data engines (RDS Postgres, ElastiCache Valkey/Redis, DocumentDB Mongo-compatible) run as labeled containers on the same DinD path. Compose does **not** publish Postgres, Redis/Valkey, or Mongo ports on the host. MQ and OpenSearch have no nested engines today (stub endpoints only); never host-publish broker or search ports.
- Lab SQL uses the **RDS Data API** HTTP facade on `:4566`. When DinD has started nested Postgres, ExecuteStatement runs real SQL via `psql` inside that container (no host DB ports). Without a nested container (or while status is still `creating`), ExecuteStatement returns `DatabaseUnavailableException` (no production SELECT stub). There is no `pgx` wire driver in the API process. Nested-network endpoint strings on Describe* responses are for DinD-side smoke only, not WAN-reachable listeners.
...
- Default Compose runs `noctaxris-engine` as privileged DinD so function containers start reliably on Docker Desktop and WSL2. Privilege stays inside that nested engine. The API container remains distroless `nonroot` without a host socket. An experimental non-privileged engine overlay lives at `docker/compose.engine-restricted.yaml` (`docker compose -f compose.yaml -f compose.engine-restricted.yaml up`); do not treat it as the packaged default until nested smoke passes on your host.
