...
How to cut a public Noctaxris release (example: **1.4.1**). Docker Hub image: **`kyaxris/noctaxris`**.
...
1. Bump `VERSION` (plain text, e.g. `1.4.1`) and keep `internal/version/version.go` (and Dockerfile `ARG VERSION`) in sync.
2. Move CHANGELOG notes under `## 1.4.1` (feature-oriented sections; no internal delivery labels).
...
git tag -a v1.4.1 -m "Release Noctaxris 1.4.1"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris:1.4.1` | Exact semver |
| `kyaxris/noctaxris:1.4` | Major.minor |
| `kyaxris/noctaxris:1` | Major |
| `kyaxris/noctaxris:latest` | Latest tagged release |
| `kyaxris/noctaxris:sha-<short>` | Git short SHA |

Then create the GitHub Release for `v1.4.1` (UI or `gh release create v1.4.1 --notes-file ...`) using the CHANGELOG `1.4.1` section.
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.4.1 -t kyaxris/noctaxris:local .
