...
How to cut a public Noctaxris release (example: **1.1.1**). Docker Hub image: **`kyaxris/noctaxris`**.
...
1. Bump `VERSION` (plain text, e.g. `1.1.1`) and keep `internal/version/version.go` default in sync.
...
git tag -a v1.1.1 -m "Noctaxris 1.1.1"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris:1.1.1` | Exact semver |
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.1.1 -t kyaxris/noctaxris:local .
