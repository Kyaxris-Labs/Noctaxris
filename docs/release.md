...
How to cut a public Noctaxris release (example: **1.3.0**). Docker Hub image: **`kyaxris/noctaxris`**.
...
1. Bump `VERSION` (plain text, e.g. `1.3.0`) and keep `internal/version/version.go` default in sync.
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris:1.3.0` | Exact semver |
| `kyaxris/noctaxris:1.3` | Major.minor |
| `kyaxris/noctaxris:1` | Major |
| `kyaxris/noctaxris:latest` | Latest tagged release |
| `kyaxris/noctaxris:sha-<short>` | Git short SHA |

Then create the GitHub Release for `v1.3.0` (UI or `gh release create v1.3.0 --notes-file ...`) using the CHANGELOG `1.3.0` section.
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.3.0 -t kyaxris/noctaxris:local .
