...
How to cut a public Noctaxris release (example: **1.5.0**). Docker Hub image: **`kyaxris/noctaxris`**.
...
1. Bump `VERSION` (plain text, e.g. `1.5.0`) and keep `internal/version/version.go` (and Dockerfile `ARG VERSION`) in sync.
2. Move CHANGELOG notes under `## 1.5.0` (feature-oriented sections; no internal delivery labels).
...
git tag -a v1.5.0 -m "Release Noctaxris 1.5.0"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris:1.5.0` | Exact semver |
| `kyaxris/noctaxris:1.5` | Major.minor |
| `kyaxris/noctaxris:1` | Major |
| `kyaxris/noctaxris:latest` | Latest tagged release |
| `kyaxris/noctaxris:sha-<short>` | Git short SHA |

Then create the GitHub Release for `v1.5.0` (UI or `gh release create v1.5.0 --notes-file ...`) using the CHANGELOG `1.5.0` section.
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.5.0 -t kyaxris/noctaxris:local .
