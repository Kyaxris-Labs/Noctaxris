...
How to cut a public Noctaxris release (example: **1.6.0**). Docker Hub image: **`kyaxris/noctaxris`**.
...
1. Bump `VERSION` (plain text, e.g. `1.6.0`) and keep `internal/version/version.go` (and Dockerfile `ARG VERSION`) in sync.
2. Move CHANGELOG notes under `## 1.6.0` (feature-oriented sections; no internal delivery labels).
...
git tag -a v1.6.0 -m "Release Noctaxris 1.6.0"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris:1.6.0` | Exact semver |
...
Then create the GitHub Release for `v1.6.0` (UI or `gh release create v1.6.0 --notes-file ...`) using the CHANGELOG `1.6.0` section.
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.6.0 -t kyaxris/noctaxris:local .
