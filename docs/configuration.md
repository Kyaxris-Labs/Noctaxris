...
| `NOCTAXRIS_IOT_ENDPOINT_HOST` | empty | Host used in `iot:DescribeEndpoint` `endpointAddress`. Empty or an IP / `localhost` returns `127.0.0.1:4566` (or that IP) for every endpoint type. A DNS hostname yields `data.iot.`, `data-ats.iot.`, `jobs.iot.`, and `credentials.iot.` prefixes on port `4566` for hosts-file and TLS SNI labs. |
