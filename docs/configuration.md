...
| `NOCTAXRIS_ALLOW_NONLOOPBACK_LISTEN` | empty | Set to `1` to allow non-loopback bind without TLS. Compose sets this for the container `0.0.0.0` binds while host publish stays `127.0.0.1:4566` and `127.0.0.1:8443`. Prefer TLS for any intentional non-loopback exposure. |
...
| `NOCTAXRIS_PUBLISH_ADDR` | `127.0.0.1` | Host address for Compose port publish (`${NOCTAXRIS_PUBLISH_ADDR}:4566:4566` and `:8443:8443`). Use `0.0.0.0` only for short host-gateway lab sessions on Docker Desktop DinD; prefer TLS if exposing beyond loopback. |
...
| `NOCTAXRIS_IOT_ENDPOINT_HOST` | empty | Host used in `iot:DescribeEndpoint` `endpointAddress`. Empty or an IP / `localhost` returns `127.0.0.1` (or that IP) for every endpoint type. A DNS hostname yields `data.iot.`, `data-ats.iot.`, `jobs.iot.`, and `credentials.iot.` prefixes for hosts-file and TLS SNI labs. Port is `4566` unless `NOCTAXRIS_IOT_TLS_LISTEN` is set, then DescribeEndpoint uses that listener's port. |
| `NOCTAXRIS_IOT_TLS_LISTEN` | empty | Dedicated IoT device HTTP / credentials TLS bind. Empty leaves those APIs on the main listener (device mTLS only if `NOCTAXRIS_TLS_CERT` / `NOCTAXRIS_TLS_KEY` are also set). Compose sets `0.0.0.0:8443` with loopback host publish. `ClientAuth` is `VerifyClientCertIfGiven` against the lab IoT CA so Python and `curl --cert` present the device cert. Non-loopback binds need `NOCTAXRIS_ALLOW_NONLOOPBACK_LISTEN=1`. Control plane and health stay HTTP `:4566`. |
...
`DescribeEndpoint` (`GET /endpoint?endpointType=` or JSON 1.1 `AWSIotService.DescribeEndpoint`) returns lab `endpointAddress` values. Port is `4566` unless `NOCTAXRIS_IOT_TLS_LISTEN` is set, then that listener's port. Types: `iot:Data`, `iot:Data-ATS`, `iot:Jobs`, `iot:CredentialProvider`. Set `NOCTAXRIS_IOT_ENDPOINT_HOST` when you need distinct `data-ats.iot.` / `jobs.iot.` / `credentials.iot.` hostnames for hosts-file SNI labs.
