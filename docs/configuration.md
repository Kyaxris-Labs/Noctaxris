...
| `NOCTAXRIS_NESTED_PORT_PUBLISH` | disabled | Set to `1` so nested data containers bind their listen port on the DinD engine host (`noctaxris-engine`). Prefer `docker/compose.lab-nested-ports.yaml`, which also publishes selected ports to `127.0.0.1` on the operator host. Default Compose leaves this off (no host DB/cache/broker ports). Mapping ports on the API service alone cannot reach DinD-internal engines. |
