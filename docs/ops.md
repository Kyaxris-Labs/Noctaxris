...
3. Pull a Hub tag (`docker pull kyaxris/noctaxris:1.3.0`) or rebuild (`docker compose ... up --build`).
...
| release (separate workflow) | Push tag `v*`: run required CI gates (`ci-required.yml`), then push semver + `latest` (+ sha) only if gates pass. Same secrets. See [release.md](release.md) |
