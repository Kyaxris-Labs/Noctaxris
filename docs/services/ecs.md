...
| Services | `CreateService`, `UpdateService`, `DeleteService`, `DescribeServices`, `ListServices` with DesiredCount lab reconciler (start/stop nested tasks toward desired). No awsvpc ENI; `RunTask` rejects non-empty `networkConfiguration.awsvpcConfiguration` |
