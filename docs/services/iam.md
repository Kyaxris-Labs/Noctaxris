...
List APIs authorize against type wildcards (`arn:aws:iam::ACCOUNT:user/*`, `role/*`, `group/*`, `policy/*`, `instance-profile/*`, IdP wildcards). `GetAccountSummary`, `GenerateCredentialReport`, and `GetCredentialReport` have no resource type, so the request resource is `*`. A statement whose Resource is only `arn:aws:iam::ACCOUNT:root` does not match those APIs. Named mutation APIs still bind the specific user/role/group/policy ARN from the request.

On dataplane resource policies (S3, SQS, SNS, Lambda, Secrets Manager), Principal `arn:aws:iam::ACCOUNT:root` or the 12-digit account id names the account root principal, not every IAM user or role in that account, and not anonymous. Same-account identity-or-resource Allow therefore needs a named user/role/session ARN or `"*"` / `{"AWS":"*"}`. Role trust and KMS key policies still treat account `:root` as delegation to that account (AND with identity). Resource-policy-only Allow still applies session and permissions-boundary explicit Deny.
