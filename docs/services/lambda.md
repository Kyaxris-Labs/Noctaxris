...
Amazon MQ event source mapping. Create only when the broker is `RUNNING` with a nested non-`stub://` endpoint (see [mq.md](mq.md)). Skip without DinD / nested broker. Unit tests inject `MQReceiveFunc`. The default path dials only allowlisted `noctaxris-mq-*` hosts, then returns an empty batch (no full AMQP client in-tree; RabbitMQ vs ActiveMQ protocol split keeps a shared client out of lab scope). Live smoke proves Create/poll wiring when the broker is RUNNING.
...
- Full AMQP/JMS consumer depth beyond lab nested-host dial + injectable receive (lab limit: default empty batch after allowlisted dial; see MQ ESM row)
