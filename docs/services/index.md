...
| [CodeBuild](codebuild.md) ([lab image](codebuild-lab-image.md)) | Shipped | Project CRUD, StartBuild/StartBuildBatch, StopBuild, BatchGetBuilds/ListBuilds; override-lock; sources `NO_SOURCE`/`S3`/`CODECOMMIT`; S3 artifacts (logs-derived); lab IMDS `FULL_URI` on `:9254`; lab webhooks (store-seed + POST receive); optional tool image under `docker/codebuild-lab/` |
