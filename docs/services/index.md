...
| [SSM Parameter Store](ssm.md) | Shipped | String, StringList, and SecureString, GetParametersByPath hierarchy, KMS via alias/aws/ssm, identity authz; Run Command lite (SendCommand/GetCommandInvocation/ListCommandInvocations, AWS-RunShellScript on nested EC2) |
...
| [ELB v2](elbv2.md) | Shipped | ALB + NLB lite: target group / listener; ALB path + host-header rules and `/alb/...` Lambda lab listener; NLB TCP/TLS control-plane + `/nlb/...` HTTP shim to ip/instance targets |
...
| [Cost and Usage Reports](cur.md) | Shipped | Report definition CRUD; optional CSV Put to S3; Parquet via nested DuckDB (`noctaxris-lab-duck` / `NOCTAXRIS_DUCKDB_URL`) |
...
| [EMR](emr.md) | Shipped | RunJobFlow / Describe / List / Terminate **control-plane stub**; AddJobFlowSteps / DescribeStep / ListSteps (immediate COMPLETED; no Spark/Hadoop) |
