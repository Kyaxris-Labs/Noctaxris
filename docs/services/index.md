...
| [IoT Core / Data](iot.md) | Shipped | Things, lab CA-signed certs, policies, principals; Topic Rules; HTTP shadows; `DescribeEndpoint` / Jobs HTTP / credentials provider mTLS on Compose `:8443`; HTTP `Publish` with `retain=true` (JSON 1.1 and REST `/topics/{topic}`) plus `ListRetainedMessages` summaries (`topic`, `qos`, `lastModifiedTime`, `payloadSize`); opt-in MQTT (`NOCTAXRIS_SHARED_MQTT`; CONNECT ClientId equals thing name) |
