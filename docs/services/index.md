...
| [SSM Parameter Store](ssm.md) | Shipped | String, StringList, and SecureString, GetParametersByPath hierarchy, version labels + history, KMS via alias/aws/ssm, identity authz; Run Command lite (SendCommand/GetCommandInvocation/ListCommandInvocations, AWS-RunShellScript on nested EC2) |
...
| [SES](ses.md) | Shipped | Local catcher: verify, SendEmail/SendRawEmail, ListIdentities, SetIdentityNotificationTopic Bounce, GetSendStatistics; SES v2 REST `/v2/email/*` identities + outbound-emails + GetAccount (shared store; no SMTP) |
...
| [Step Functions](stepfunctions.md) | Shipped | State machine CRUD, StartExecution, Pass/Succeed/Fail/Choice (Equals + GreaterThan/LessThan/IsPresent)/Wait (0–5s)/Parallel/Map (sequential)/Task to Lambda/SQS/SNS/EventBridge; waitForTaskToken + SendTaskSuccess/Failure/Heartbeat; InputPath/ResultPath; lab resource policy for EventBridge RoleArn-less StartExecution |
...
| [Glue](glue.md) | Shipped | Data Catalog database and table CRUD (PartitionKeys + SerDe fields for Athena); Schema Registry lite (registries/schemas/versions; SchemaReference column resolve); crawler lite Create/Start/Get/Delete/List |
| [WAF v2](wafv2.md) | Shipped | WebACL / rule group / IPSet CRUD; AssociateWebACL (HTTP API / AppSync / Lambda / ALB); ByteMatch / SizeConstraint / IPSet (inline or ARN); invoke DefaultAction gate; Evaluate helper |
...
| [AppSync](appsync.md) | Shipped | GraphQL API CRUD, schema status, Lambda DS/resolver management SAR (optional PassRole), nested selections (depth ≤ 3), API_KEY/IAM/Cognito auth |
| [API Gateway REST API](apigateway.md) | Shipped | REST v1 CreateRestApi/resource/method, PutIntegration AWS_PROXY+MOCK, CreateDeployment/Stage, TOKEN/REQUEST authorizers, usage plans/API keys lite, execute `/restapis/{id}/{stage}/_user_request_/...`, NONE/AWS_IAM/CUSTOM |
...
| [Cognito User Pools](cognito-idp.md) | Shipped | Pool/client CRUD (UpdateUserPool), AdminCreateUser / AdminGetUser / AdminSetUserPassword / AdminDeleteUser / AdminDisableUser / ListUsers, USER_PASSWORD_AUTH / USER_SRP_AUTH / CUSTOM_AUTH (optional SRP nesting) / refresh / RevokeToken, ForgotPassword / ConfirmForgotPassword / ResendConfirmationCode / UpdateUserAttributes / attribute verify, TOTP SOFTWARE_TOKEN_MFA, lab RoleArn + LambdaConfig PassRole and sync trigger Invoke (PostConfirmation / PreTokenGeneration / PreSignUp / Pre+PostAuthentication / CustomMessage SignUp/AdminCreateUser/ForgotPassword/ResendCode/UpdateUserAttribute/VerifyUserAttribute / UserMigration password / custom-auth Define/Create/Verify), RS256 tokens, JWKS on loopback |
| [CloudFront](cloudfront.md) | Shipped | Distribution CRUD + GetDistributionConfig/UpdateDistribution (ETag/IfMatch); invalidation theatre (Completed immediately); Deployed + lab DomainName; CacheBehavior PathPattern → TargetOriginId lite; optional Logging to in-account S3 on edge GET; SigV4 edge GET `/cloudfront/{id}/...` to S3 or HTTP API origin (no real PoP) |
| [ELB v2](elbv2.md) | Shipped | ALB + NLB lite: target group / listener; ALB path/host/http-header/query/source-ip rules + ModifyListener/ModifyRule; `/alb/...` Lambda lab listener; NLB TCP/TLS control-plane + `/nlb/...` HTTP shim to ip/instance targets |
...
| [IoT Core / Data](iot.md) | Shipped | Things, lab CA-signed certs, policies, principals; Topic Rules; HTTP shadows; `DescribeEndpoint` / Jobs HTTP / credentials provider mTLS on Compose `:8443`; HTTP `Publish` with `retain=true` (JSON 1.1 and REST `/topics/{topic}`); `GetRetainedMessage` (JSON 1.1 and REST `/retainedMessage/{topic}`); `ListRetainedMessages` summaries (`topic`, `qos`, `lastModifiedTime`, `payloadSize`); opt-in MQTT (`NOCTAXRIS_SHARED_MQTT`; CONNECT ClientId equals thing name) |
...
| [RDS Data API](rds-data.md) | Shipped | ExecuteStatement / BatchExecuteStatement for `postgres`, `mysql`, `mariadb`; Begin/Commit/Rollback via held wire sessions; Postgres prefers `pgx` else nested `psql`; MySQL/MariaDB prefer `go-sql-driver/mysql` else nested `mysql` CLI; named `parameters`; `formatRecordsAs=JSON`; Postgres Batch `generatedFields` from `RETURNING`; secretArn fail-closed |
...
| [Neptune](neptune.md) | Shipped | neptune Create/Describe/Delete; nested Gremlin `:8182` default; opt-in Neo4j Bolt `:7687` (`NOCTAXRIS_NEPTUNE_ENGINE` / tag); nested-network endpoint only |
| [Athena](athena.md) | Shipped | Start/Get/Stop/GetQueryResults over Glue + lab S3; WorkGroup CRUD + enforce/default OutputLocation; in-process WHERE equality/`!=`/`<>`/range/`BETWEEN`/`IN`/LIKE/json_extract; COUNT(*), INNER JOIN, GROUP BY, ORDER BY; optional nested DuckDB (`noctaxris-lab-duck` / `NOCTAXRIS_DUCKDB_URL`) for Parquet + broader SQL; CloudTrail Records[] + gzip; GetObject/OutputLocation fail closed |
...
| [EMR](emr.md) | Shipped | RunJobFlow / Describe / List / Terminate **control-plane stub**; steps (Add/Describe/List/Cancel); ListInstanceGroups/Fleets; tags; security configs (no Spark/Hadoop) |
| [Bedrock Runtime](bedrock-runtime.md) | Shipped | InvokeModel + Converse allowlist **canned** JSON stub; ConverseStream 501 |
