...
| Expressions | `rate(n minutes\|hours\|days)`, AWS-shaped `cron(minutes hours day-of-month month day-of-week year)` subset (digits, `*`, `?`, lists/ranges/steps, `L`, DOM `nW` / `LW`, `#`, month `JAN-DEC` / DOW `SUN-SAT` names), optional one-time `at(yyyy-mm-ddThh:mm:ss)` |
