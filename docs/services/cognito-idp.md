...
- Confirmation codes (secure by default): `SignUp` / `ResendConfirmationCode` / `ForgotPassword` issue a high-entropy code (8+ hex chars), store it with a one-hour TTL, and require a constant-time match on `ConfirmSignUp` / `ConfirmForgotPassword`. Codes are single-use (cleared on success). Wrong, missing, expired, or reused codes return `CodeMismatchException`.
- `NOCTAXRIS_COGNITO_INSECURE_CODES=1` restores the prior lab stubs: fixed code `123456` for ForgotPassword, and any non-empty `ConfirmSignUp` code. See [configuration.md](../configuration.md).
- `UpdateUserAttributes` / `GetUserAttributeVerificationCode` / `VerifyUserAttribute` are AccessToken public IdP APIs. Lab attributes live in SQLite; `email` / `phone_number` start unverified and accept lab code `123456` on `VerifyUserAttribute`.
