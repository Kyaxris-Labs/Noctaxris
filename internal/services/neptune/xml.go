...
func CreateDBClusterXML(c store.NeptuneCluster, requestID string) ([]byte, error) {
...
func DescribeDBClustersXML(clusters []store.NeptuneCluster, requestID string) ([]byte, error) {
	type dbCluster struct {
		DBClusterIdentifier string `xml:"DBClusterIdentifier"`
		Engine              string `xml:"Engine"`
		EngineVersion       string `xml:"EngineVersion"`
...
func DeleteDBClusterXML(c store.NeptuneCluster, requestID string) ([]byte, error) {
	type dbCluster struct {
		XMLName             xml.Name `xml:"DBCluster"`
		DBClusterIdentifier string   `xml:"DBClusterIdentifier"`
		Status              string   `xml:"Status"`
...
// ErrorXML builds a Neptune error response.
...
func marshal(root string, result any, requestID string) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("marshal neptune xml: %w", err)
	}
