...
const (
	...
	EnvDockerHostAllowlist = "NOCTAXRIS_DOCKER_HOST_ALLOWLIST"

	defaultDockerHost = "tcp://noctaxris-engine:2376"
)
...
func ValidateDockerHost(dockerHost, tlsCertPath string) error {
	...
	if strings.Contains(lower, "docker.sock") {
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST must not reference docker.sock")
	}
	u, err := url.Parse(host)
	if err != nil {
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST: %w", err)
	}
	scheme := strings.ToLower(u.Scheme)
	switch scheme {
	case "unix", "npipe":
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST scheme %q is not allowed (use tcp:// to noctaxris-engine)", scheme)
	case "tcp":
		// ok
	default:
		if scheme == "" {
			return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST must be a tcp:// URL")
		}
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST scheme %q is not allowed", scheme)
	}
	if !dockerHostAllowlisted(host) {
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_HOST %q is not allowlisted (default %s; extend via %s)",
			host, defaultDockerHost, EnvDockerHostAllowlist)
	}
	certDir := strings.TrimSpace(tlsCertPath)
	if certDir == "" {
		return fmt.Errorf("compute: NOCTAXRIS_DOCKER_CERT_PATH is required when NOCTAXRIS_DOCKER_HOST is set")
	}
	for _, name := range []string{"ca.pem", "cert.pem", "key.pem"} {
		p := filepath.Join(certDir, name)
		st, err := os.Stat(p)
		if err != nil {
			return fmt.Errorf("compute: NOCTAXRIS_DOCKER_CERT_PATH: %s: %w", name, err)
		}
		if st.IsDir() || st.Size() == 0 {
			return fmt.Errorf("compute: NOCTAXRIS_DOCKER_CERT_PATH: %s must be a non-empty file", name)
		}
	}
	return nil
}
