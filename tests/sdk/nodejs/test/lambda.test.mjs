...
test("Lambda create get delete", async (t) => {
  ...
  const got = await lam.send(new GetFunctionCommand({ FunctionName: fnName }));
  assert.equal(got.Configuration?.FunctionName, fnName);

  const list = await lam.send(new ListFunctionsCommand({}));
  const names = (list.Functions || []).map((f) => f.FunctionName);
  assert.ok(names.includes(fnName), `ListFunctions missing ${fnName}`);
