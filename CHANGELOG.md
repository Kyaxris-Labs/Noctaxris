...
- IoT: `DescribeEndpoint` (`GET /endpoint?endpointType=`), REST named-shadow list (classic unnamed omitted), Jobs HTTP data plane (`iot-jobs-data`), credentials provider mTLS (`GET /role-aliases/{alias}/credentials` with matching `x-amzn-iot-thingname` and CredentialProvider SNI). Data-plane REST `/things/...` and `/api/things/...` require IoT SigV4 scope (`iot` / `iotdata` / `iotdevicegateway` / `iot-jobs-data`); `s3`-signed GetObject on those keys stays S3. Credentials GET is claimed only with mTLS (unsigned non-mTLS is 403). `ListRetainedMessages` returns persisted topic summaries (`topic`, `payloadSize`, `qos`, `lastModifiedTime`; no payload). Empty list is HTTP 200
